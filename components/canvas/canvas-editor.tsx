...
export function CanvasEditor({ className, imageSrc, onImageLoad }: CanvasEditorProps) {
  ...
  // Load image from prop when it changes
  useEffect(() => {
    if (imageSrc && actions.loadImage) {
      // Create a fake File object for the image
      const fakeFile = new File([], "uploaded-image.png", { type: "image/png" });
      actions.loadImage(imageSrc, fakeFile);
    }
  }, [imageSrc, actions]);
